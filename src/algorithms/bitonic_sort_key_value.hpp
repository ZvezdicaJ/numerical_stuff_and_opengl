 ...
 * @param end index of the last element to be sorted
 */
inline void compare_full_length(double *arr, long long *keys, unsigned start,
                                unsigned end) {
    unsigned length = end - start + 1;
    unsigned half = length / 2;
 ...
 * @param depth a parameter to follow the depth of recursion
 */
inline void lane_crossing_compare(double *arr, long long *keys, unsigned start,
                                  unsigned end, unsigned depth) {
    ...
    if (length == 4) {
        ...
        __m256i mask =
            _mm256_set_epi64x(0xffffffffffffffff, 0xffffffffffffffff,
                              0x0000000000000000, 0x0000000000000000);
        permute_and_compare(reg, key, mask, 0b01001110);

        mask = _mm256_set_epi64x(0xffffffffffffffff, 0x0000000000000000,
                                 0xffffffffffffffff, 0x0000000000000000);
        shuffle_and_compare(reg, key, mask, 0b0101);
    ...
    for (unsigned i = 0; i < length / 2; i += 4) {
        {
            ...
            __m256i key0 = _mm256_load_si256((__m256i *)k1); // i-ti od začetka
            __m256i key1 =
                _mm256_load_si256((__m256i *)k2); // ta je prvi čez polovico
 ...
 * @details end-start+1 should be 2^n
 */
inline void sort_2n_vector(double *array, long long *keys, unsigned start,
                           unsigned end) {
...
//////////////////////////////////////////////////////////////////
// KEY:VALUE LONG LONG:DOUBLE SORTING ALGORITHM, the array must contain 4n
// elements
 ...
 * @details end-start+1 should be 8*n
 */
inline void sort_4n_vector(double *array, long long *keys, unsigned start,
                           unsigned end) {
    int pow2 = (int)std::ceil(std::log2f(end + 1));
    int imaginary_length = (int)std::pow(2, pow2);
    unsigned full_length = end - start + 1;
...
inline void maskload(int diff, double *p, long long *k, __m256i &mask,
                     __m256d &reg1, __m256i &key1) {
    switch (diff) {
    case 0: {
        mask = _mm256_set_epi64x(0, 0, 0, LOAD);
        reg1 = _mm256_maskload_pd(p, mask);
        key1 = _mm256_maskload_epi64(k, mask);
        __m256d infinity =
 ...
 * @param end index of the last element to be sorted
 */
inline void compare_full_length_all_cases(double *arr, long long *keys,
                                          unsigned start, unsigned end,
                                          unsigned last_index) {
    // std::cout << "start: " << start << "end: " << end
    //          << " last index: " << last_index << std::endl;
 ...
 * @param depth a parameter to follow the depth of recursion
 */
inline void lane_crossing_compare_all_cases(double *arr, long long *keys,
                                            unsigned start, unsigned end,
                                            unsigned last_index,
                                            unsigned depth) {
    if (start > last_index) {
    ...
    if (length == 4) {
        ...
        if (diff < 3) {
            ...
            _mm256_maskstore_epi64((keys + start), load_store_mask, key);
        } else {
            _mm256_store_pd(arr + start, reg);
            _mm256_store_si256((__m256i *)(keys + start), key);
        }
 ...
 * @details end - start + 1 should be 2^n
 */
inline void sort_vector(aligned_vector<double> &array,
                        aligned_vector<long long> keys, unsigned start,
                        unsigned end) {
    unsigned full_length = end - start + 1;
    if (full_length <= 1)
        return;
    else if (full_length < 4) {
        long long *k = keys.data() + start;
        double *p = array.data() + start;
        __m256d reg1;
        __m256i mask, key1;
        maskload(full_length - 1, p, k, mask, reg1, key1);
        bitonic_sort(reg1, key1);
        _mm256_maskstore_pd(array.data() + start, mask, reg1);
        _mm256_maskstore_epi64((keys.data() + start), mask, key1);
        return;
    } else if (!(full_length & (full_length - 1)))
        sort_2n_vector(array.data(), keys.data(), start, end);
    else if (mod4(full_length) == 0)
        sort_4n_vector(array.data(), keys.data(), start, end);
    else {
        int pow2 = (int)std::ceil(std::log2f(end + 1));
        int imaginary_length = (int)std::pow(2, pow2);
        unsigned full_length = end - start + 1;
        ...
        _mm256_maskstore_epi64((keys.data() + end - reminder), mask, key1);
