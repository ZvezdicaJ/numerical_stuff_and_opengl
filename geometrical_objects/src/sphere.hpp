...
Sphere<T>::draw(float radius, std::array<float, 3> position,
    ...
    glVertexAttribPointer(0, this->vertex_size, GL_FLOAT, GL_TRUE,
                          this->vertex_size * sizeof(float), (void *)0);
    glEnableVertexAttribArray(0);
    glDrawElements(GL_TRIANGLES, this->element_array.size(), GL_UNSIGNED_INT,
                   0);
...
Sphere<T>::draw(float radius, std::array<float, 3> position,
  ...
  glUseProgram(this->shaderProgram);
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * (this->vertex_colors.size()),
                 &(this->vertex_colors[0]), GL_STATIC_DRAW);
